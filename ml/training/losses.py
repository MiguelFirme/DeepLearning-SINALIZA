...
def compute_class_weights(
    labels: list[int] | np.ndarray, num_classes: int,
    strategy: str = "inverse", effective_beta: float = 0.999,
) -> torch.Tensor:
    """
    ...
    Args:
        ...
        effective_beta: beta usado pela estratégia 'effective'
    ...
    if np.any(counts == 0):
        raise ValueError("Pesos de classe exigem ao menos uma amostra de treino por classe")
    ...
    if strategy == "inverse":
        ...
        if not 0 <= effective_beta < 1:
            raise ValueError("effective_beta deve estar no intervalo [0, 1)")
        weights = (1 - effective_beta) / (1 - np.power(effective_beta, counts))
    else:
